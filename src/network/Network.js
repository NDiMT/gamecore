...
class NetworkSingleton {
  constructor() {
    ...
    this.myCode = ""; this.listeners = new Set(); this.connected = false;
  }
  on(fn) { this.listeners.add(fn); return () => this.listeners.delete(fn); }
  emit(ev) { for (const fn of this.listeners) fn(ev); }
  host(onCode) {
    this.myCode = randCode(); this.isHost = true;
    this.peer = new Peer(ROOM_PREFIX + this.myCode, { config: { iceServers: ICE } });
  ...
  join(code, onStatus) {
    this.isHost = false; onStatus && onStatus("Σύνδεση…");
    this.peer = new Peer(ROOM_PREFIX + "j-" + randCode(), { config: { iceServers: ICE } });
    this.peer.on("open", () => {
      onStatus && onStatus("Αναζήτηση…");
      this.conn = this.peer.connect(ROOM_PREFIX + code, { reliable: true });
      this.wire();
      setTimeout(() => { if (!this.connected) onStatus && onStatus("Δεν συνδέθηκε. Δοκίμασε ίδιο WiFi."); }, 20000);
    });
  ...
  wire() {
    this.conn.on("open", () => { this.connected = true; this.emit({ type: "open" }); });
  ...
  send(type, payload) {
    ...
    try { this.conn.send({ type, payload }); } catch (e) {}
  }
}
