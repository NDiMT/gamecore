...
signal pick_received(id: int, cls: String, name: String, groups: Array)
...
signal shop_received(shop: Dictionary)
...
func send_pick(cls: String, name: String, groups: Array) -> void:
	_pick.rpc_id(1, cls, name, groups)
...
@rpc("any_peer", "call_remote", "reliable")
func _pick(cls: String, name: String, groups: Array) -> void:
	pick_received.emit(multiplayer.get_remote_sender_id(), cls, name, groups)
...
func broadcast_shop(shop: Dictionary) -> void:
	_shop.rpc(shop)
